// src/data/blog.js
const blogData = {
  name: 'Underreacted',
  image: 'data:image/png;base64,',
  about: 'A blog about learning React',
  posts: [
    {
      id: 1,
      title: 'Getting Started with React',
      date: 'August 10, 2024',
      preview: 'This post covers the basics of setting up a React application and the core concepts you need to know.',
      minutesToRead: 7
    },
    {
      id: 2,
      title: 'Understanding Components',
      date: 'August 11, 2024',
      preview: 'Learn about React components, including functional and class components, and how to use them effectively.',
      minutesToRead: 35
    },
    {
      id: 3,
      title: 'State and Lifecycle in React',
      date: 'August 12, 2024',
      preview: 'Dive into how state and lifecycle methods work in React and how they can be used to build dynamic applications.',
      minutesToRead: 15
    },
    {
      id: 4,
      title: 'Handling Events in React',
      date: 'August 13, 2024',
      preview: 'Understand how to handle events in React components and manage user interactions effectively.',
      minutesToRead: 10
    },
    {
      id: 5,
      title: 'React Hooks: An Introduction',
      date: 'August 14, 2024',
      preview: 'Explore the power of React Hooks, including useState, useEffect, and custom hooks for managing state and side effects.',
      minutesToRead: 20
    },
    {
      id: 6,
      title: 'Context API for State Management',
      date: 'August 15, 2024',
      preview: 'Learn about the Context API and how it can be used to manage global state in your React application.',
      minutesToRead: 25
    },
    {
      id: 7,
      title: 'Optimizing Performance in React',
      date: 'August 16, 2024',
      preview: 'Tips and techniques for optimizing performance in React applications to ensure a smooth user experience.',
      minutesToRead: 18
    },
    {
      id: 8,
      title: 'Testing React Components',
      date: 'August 17, 2024',
      preview: 'An introduction to testing React components using popular testing libraries and best practices for writing tests.',
      minutesToRead: 22
    }
  ]
};

export default blogData;
